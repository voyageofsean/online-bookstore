...
def search(request):
    ...
    if request.method == 'POST':
        ...
        #查询所有可能结果
        book_list = (Book.objects.filter(ISBN__icontains=query)|Book.objects.filter(BookName__icontains=query)|Book.objects.filter(BookAuthor__icontains=query)).distinct()
        if not book_list:
            error_msg = '没找到所需的书籍'
...
#书籍详细信息功能
def bookInformation(request,isbn):
    book=Book.objects.filter(ISBN=isbn)
    return render(request, 'bookInformation.html', {'book':book})
...
def showBook(request):
    ...
    return render(request, 'showBook.html', {'books':selected_list})